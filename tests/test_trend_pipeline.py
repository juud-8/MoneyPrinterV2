...
class _FrozenNow(datetime):
    """datetime pinned to NOW, so seeds built for these fixtures never age out.

    Tests that drive production code paths (generate_topic) cannot pass an
    explicit `now` — expiry there is deliberately checked against the wall
    clock. Without this the fixtures pass only until EXPIRES, then fail
    forever.
    """
...
def frozen_pipeline_clock():
    return patch("trend_pipeline.datetime", _FrozenNow)
...
class TrendPipelineTests(unittest.TestCase):
    ...
    def test_duplicate_gate_still_rejects_seed(self):
        ...
        ), patch("classes.YouTube.log_topic_rejection"), frozen_pipeline_clock():
